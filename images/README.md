...
**Current status:**
- `hero/`, `categories/`, and `articles/` already have generated artwork
  in place (soft, on-brand abstract compositions — not real photos).
- `products/` is intentionally **empty**. Every product card falls back
  automatically to a clean placeholder tile (generated in the browser,
  no file needed) until you add real product photos.

Whenever you're ready, replace any of these files with a real photo
using the **exact same filename**, and it'll appear automatically — no
code changes needed. There's also a JS-based fallback for any image
that's missing or fails to load, so the site stays resilient either way.
...
├── products/                → empty by design — add real product photos here
